...
var taxonomy_name = currentLocation.pathname.match(taxonomyPattern)
    ? currentLocation.pathname.match(taxonomyPattern).join()
    : "";
var is_taxonomy = "";
if (taxonomy_name) {
...
var getRequest_price = currentLocation.search.match(pricePattern)
    ? currentLocation.search.match(pricePattern).join()
    : "";
...
$.getJSON(
    productsData.root_url +
        "/wp-json/api/v1/products" +
        currentLocation.search +
        is_taxonomy +
        taxonomy_name,
    ($results) => {
        $("#results-counter").html(`
    ${$results.paginationInfo.map(
        (item) =>
            `<p>Showing ${item.show_start} - ${item.show_end} of ${item.total_posts} results</p> `
    )}`);
    ...
    <a href="${
        item.permalink
    }" class="flex flex-col items-center justify-center space-y-4 bg-white border rounded-b-xl hover:bg-gray-100">
        <div class="w-full" id="product card">
            <img src="${item.image}" alt="" class="w-full">
            <h1 class="flex justify-center font-bold text-gray-600" id="product-name">${
                item.title
            }</h1>
